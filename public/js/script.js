...
document.getElementById('star-button').addEventListener('click', function () {
  ...
  tween.to({
    ...
    .duration(9000) // Slower transition
    .easing(TWEEN.Easing.Cubic.InOut)
    .onUpdate(({ radius, spin, randomness, rotate }) => {
    ...
    .onComplete(() => {
      new TWEEN.Tween(camera.position)
        .to({ x: 0, y: 1, z: 1 }, 9000) // Slower transition
        .easing(TWEEN.Easing.Cubic.InOut)
        .onUpdate(() => {
  ...
  setTimeout(function () {
    const starContainer = document.getElementById('icon-container')
